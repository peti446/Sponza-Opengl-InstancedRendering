...
void Context::update()
{
    ...
    lights_[0].setPosition(Vector3(50.f * cosf(time_seconds_), 50.f, 0.f));
    lights_[0].setRange(70.f);
    for (int i = 1; i < num_of_lights; ++i) {
        auto& light = lights_[i];
		float A = time_seconds_ + i * 6.28f / (num_of_lights - 1);
